    ...
    for(int x = 0;x < 256;x++)
    {
       ...
       if(gettrans == -1)
       {
          if(palette[x].red==0 && palette[x].green==255 && palette[x].blue==255)
             gettrans = x;
          else if(palette[x].red==255 && palette[x].green==0 && palette[x].blue==255)
             gettrans = x;
...
void ExtraerSKYS(int idx, int num)
{
    ...
    int pngsize = 0;
    cache pngout = Png_Create(input, &pngsize, 512, 512, idx-1, 0, 0, true);
...
void ShowInfo()
{
    ...
    setcolor(0x07);printf("#############\n"); 
    printf("     # WOLFENSTEIN RPG IPHONE EXTRACTOR#\n");
    printf("     # CREADO POR ERICK VASQUEZ GARCIA #\n");
    printf("     # ES PARA WOLFENSTEIN RPG (IPHONE)#\n");
...
void ExtraerDatos()
{
    hz = OpenZip(("Wolfenstein RPG 1.0.2.ipa"),0);
    if(hz == NULL){error("No encuentra Wolfenstein RPG 1.0.2.ipa");}
    SetUnzipBaseDir(hz,("Datos"));
    ZIPENTRY ze; GetZipItem(hz,-1,&ze); int numitems=ze.index;
    
    int i;
    FindZipItem(hz,("Payload/Wolf...RPG.app/Packages/newMappings.bin"),true,&i,&ze);
    UnzipItem(hz,i,ze.name+32);
    FindZipItem(hz,("Payload/Wolf...RPG.app/Packages/newPalettes.bin"),true,&i,&ze);
    UnzipItem(hz,i,ze.name+32);
    FindZipItem(hz,("Payload/Wolf...RPG.app/Packages/tables.bin"),true,&i,&ze);
    UnzipItem(hz,i,ze.name+32);
    
    for(int j = 0; j <= 20; j++)
    {
        PrintfPorcentaje(j,20-1,true, 10,"Extrayendo Datos......");
        sprintf(string,"Payload/Wolf...RPG.app/Packages/newTexels%3.3d.bin",j);
...
int main(int argc, char *argv[])
{
    ...
    hz = CreateZip("WolfensteinRpgIphone.pk3",0);
